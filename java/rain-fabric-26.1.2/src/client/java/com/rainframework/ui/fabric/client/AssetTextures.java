...
final class AssetTextures {
    ...
    @Nullable Identifier texture(String hash, long nowMillis) {
    ...
    private @Nullable Frames loadGif(String hash, ByteBuffer encoded) {
        try (final var stack = MemoryStack.stackPush()) {
            ...
            try {
                ...
                final var ids = new ArrayList<Identifier>();
    ...
    // NativeImage takes ARGB in this version, while stb gives RGBA bytes.
    private static Identifier upload(String hash, int frame, ByteBuffer rgba, int offset, int width, int height) {
        ...
        final var id = Identifier.fromNamespaceAndPath("rain-ui", "asset/" + hash + "/" + frame);
    ...
    private record Frames(List<Identifier> ids, int[] durations, int width, int height) {

        Identifier at(long nowMillis) {
            if (ids.size() == 1) {
                return ids.getFirst();
            }
